...
from z_sandpit.test_data.configuration.z_sandpit_test_constants import (
...
class TestHuggingFaceFineTunedModelBoro:
    ...
    def setup(self):
        self.pdf_folder = os.path.join(Z_SANDPIT_TEST_DATA_FOLDER_PATH, "inputs")

        self.z_sandpit_outputs_folder = os.path.join(
            Z_SANDPIT_TEST_DATA_FOLDER_PATH, "outputs"
        )

        self.models_folder_path = os.path.join(self.z_sandpit_outputs_folder, "models")
        ...
        self.pretrained_model_name_or_path = os.path.join(
            self.z_sandpit_outputs_folder, "models", "accounting_fine_tuned_model"
        )
        ...
        LogFiles.open_log_file(
            folder_path=os.path.join(self.z_sandpit_outputs_folder, "logs")
        )
    ...
    def test_tokenisation(self):
        ...
        self.tokenizer.print_tokenized_data(num_samples=8)
