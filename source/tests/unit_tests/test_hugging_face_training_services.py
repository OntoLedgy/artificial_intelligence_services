...
class TestHuggingFaceFineTunedModel:
    ...
    def setup(self):
        ...
        self.model_folder_path = os.path.join(
            self.models_folder_path, self.model_name, "fine_tuned_model"
        )

        self.tokeniser_folder_path = os.path.join(
            self.models_folder_path, self.model_name, "fine_tuned_tokeniser"
        )
    ...
    def test_text_generation(self):
        model, tokeniser = load_model(self.model_name, self.models_folder_path)

        generate_text_using_pipeline(model, tokeniser, self.prompt)

        generate_text_using_model(model, tokeniser, self.prompt)
