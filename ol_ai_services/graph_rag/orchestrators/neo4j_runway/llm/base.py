...
class BaseDataModelingLLM(ABC):
    ...
    def _get_data_model_response(
        ...
        # Ensure DataModel has required attributes
        if not hasattr(response, "nodes") or response.nodes is None:
            response.__dict__["nodes"] = []
        if not hasattr(response, "relationships") or response.relationships is None:
            response.__dict__["relationships"] = []
